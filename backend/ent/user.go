...
type UserEdges struct {
	...
	// BenefitClaims holds the value of the benefit_claims edge.
	BenefitClaims []*BenefitClaim `json:"benefit_claims,omitempty"`
...
// BenefitClaimsOrErr returns the BenefitClaims value or an error if the edge
// was not loaded in eager-loading.
func (e UserEdges) BenefitClaimsOrErr() ([]*BenefitClaim, error) {
	if e.loadedTypes[10] {
		return e.BenefitClaims, nil
	}
	return nil, &NotLoadedError{edge: "benefit_claims"}
}
...
// QueryBenefitClaims queries the "benefit_claims" edge of the User entity.
func (_m *User) QueryBenefitClaims() *BenefitClaimQuery {
	return NewUserClient(_m.config).QueryBenefitClaims(_m)
}
