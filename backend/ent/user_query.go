...
type UserQuery struct {
	...
	withBenefitClaims         *BenefitClaimQuery
...
// QueryBenefitClaims chains the current query on the "benefit_claims" edge.
func (_q *UserQuery) QueryBenefitClaims() *BenefitClaimQuery {
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withBenefitClaims:         _q.withBenefitClaims.Clone(),
...
// WithBenefitClaims tells the query-builder to eager-load the nodes that are connected to
// the "benefit_claims" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithBenefitClaims(opts ...func(*BenefitClaimQuery)) *UserQuery {
	query := (&BenefitClaimClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withBenefitClaims = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withBenefitClaims; query != nil {
		if err := _q.loadBenefitClaims(ctx, query, nodes,
			func(n *User) { n.Edges.BenefitClaims = []*BenefitClaim{} },
			func(n *User, e *BenefitClaim) { n.Edges.BenefitClaims = append(n.Edges.BenefitClaims, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadBenefitClaims(ctx context.Context, query *BenefitClaimQuery, nodes []*User, init func(*User), assign func(*User, *BenefitClaim)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(benefitclaim.FieldUserID)
	}
	query.Where(predicate.BenefitClaim(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.BenefitClaimsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
