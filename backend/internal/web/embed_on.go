...
func (s *FrontendServer) serveIndexHTML(c *gin.Context) {
	...
	baseHTML, isOverride := s.getIndexHTML()
	useBaseCache := !isOverride
...
func (s *FrontendServer) getIndexHTML() ([]byte, bool) {
	if s.overrideDir == "" {
		return s.baseHTML, false
	}
	content, err := os.ReadFile(filepath.Join(s.overrideDir, "index.html"))
	if err != nil {
		return s.baseHTML, false
	}
	return content, true
...
func injectSiteTitle(html, settingsJSON []byte) []byte {
	...
	newTitle := []byte("<title>" + htmlpkg.EscapeString(defaultSEOTitle+" - "+cfg.SiteName) + "</title>")
	var buf bytes.Buffer
