...
func TestSecurityHeaders(t *testing.T) {
	...
	t.Run("csp_disabled_still_allows_image_playground_shell_same_origin_embed", func(t *testing.T) {
		cfg := config.CSPConfig{Enabled: false}
		middleware := SecurityHeaders(cfg, nil)

		for _, path := range []string{"/image-playground", "/image-playground/"} {
			w := httptest.NewRecorder()
			c, _ := gin.CreateTestContext(w)
	...
	t.Run("image_playground_static_app_can_be_embedded_by_same_origin", func(t *testing.T) {
		cfg := config.CSPConfig{
			Enabled: true,
