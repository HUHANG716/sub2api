...
func TestUsageLogRepositoryGetStatsWithFiltersRequestedModelSource(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs\\s+WHERE COALESCE\\(NULLIF\\(TRIM\\(requested_model\\), ''\\), model\\) = \\$1.*GROUP BY GROUPING SETS").
		...
		WillReturnRows(sqlmock.NewRows([]string{
			...
			AddRow(1, 1, nil, nil, int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 0.2, 1.2, 20.0).
			AddRow(0, 1, "/v1/responses", nil, int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 0.2, 1.2, 20.0).
			AddRow(1, 0, nil, "/v1/responses", int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 0.2, 1.2, 20.0).
			AddRow(0, 0, "/v1/responses", "/v1/responses", int64(1), int64(2), int64(3), int64(1), int64(3), 1.2, 1.0, 0.2, 1.2, 20.0))

	stats, err := repo.GetStatsWithFilters(context.Background(), filters)
	require.NoError(t, err)
	require.Equal(t, int64(1), stats.TotalRequests)
	require.InDelta(t, 0.2, stats.TotalDiscountAmount, 1e-12)
...
func TestUsageLogRepositoryGetStatsWithFiltersAlwaysReturnsAccountCost(t *testing.T) {
	...
	mock.ExpectQuery("(?s)FROM usage_logs.*GROUP BY GROUPING SETS").
		WillReturnRows(sqlmock.NewRows([]string{
			"inbound_grouped", "upstream_grouped", "inbound_endpoint", "upstream_endpoint",
			"requests", "input_tokens", "output_tokens", "cache_creation_tokens", "cache_read_tokens",
			"cost", "actual_cost", "discount_amount", "account_cost", "avg_duration_ms",
		}).AddRow(1, 1, nil, nil, int64(50), int64(1000), int64(2000), int64(60), int64(40), 15.0, 12.5, 2.5, 11.0, 100.0))

	stats, err := repo.GetStatsWithFilters(context.Background(), filters)
	require.NoError(t, err)
	require.NotNil(t, stats.TotalAccountCost, "TotalAccountCost must always be returned, even without AccountID filter")
	require.InDelta(t, 2.5, stats.TotalDiscountAmount, 1e-12)
...
func TestUsageLogRepositoryGetUserSpendingRankingUserRoleFilter(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"user_id", "email", "username", "actual_cost", "requests", "tokens", "total_actual_cost", "total_requests", "total_tokens"}).
		AddRow(int64(9), "admin@example.com", "admin", 21.0, int64(4), int64(500), 21.0, int64(4), int64(500))
	...
	require.Equal(t, []usagestats.UserSpendingRankingItem{
		{UserID: 9, Email: "admin@example.com", Username: "admin", ActualCost: 21.0, Requests: 4, Tokens: 500},
	}, got.Ranking)
