...
func TestCreateOrderInTx_WritesBalanceOrderWhenPeriodLimitColumnMissing(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)

	user, err := client.User.Create().
		SetEmail("balance-snapshot@example.com").
		SetPasswordHash("hash").
		SetUsername("balance-snapshot-user").
		Save(ctx)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Nil(t, order.ProviderSnapshot)
}

func TestCreateOrderInTx_WritesSubscriptionPeriodLimitSnapshot(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)
	ensureSubscriptionPlanPeriodLimitColumn(t, client)
	...
	plan, err := client.SubscriptionPlan.Create().
		...
		Save(ctx)
	require.NoError(t, err)
	_, err = client.ExecContext(ctx, "UPDATE subscription_plans SET period_limit_usd = ? WHERE id = ?", 500.0, plan.ID)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, 500.0, order.ProviderSnapshot["subscription_period_limit_usd"])
}

func TestCreateOrderInTx_CreatesSubscriptionOrderWhenPeriodLimitColumnMissing(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)

	user, err := client.User.Create().
		SetEmail("subscription-snapshot-missing@example.com").
		SetPasswordHash("hash").
		SetUsername("subscription-snapshot-missing-user").
		Save(ctx)
	require.NoError(t, err)
...
func ensureSubscriptionPlanPeriodLimitColumn(t *testing.T, client *dbent.Client) {
	t.Helper()
	_, err := client.ExecContext(context.Background(), "ALTER TABLE subscription_plans ADD COLUMN period_limit_usd DECIMAL(20, 10)")
	require.NoError(t, err)
}
