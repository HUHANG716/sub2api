...
func TestValidatePlanRequired_NegativePeriodLimit(t *testing.T) {
	neg := -1.0
	err := validatePlanRequired("Pro", 1, 9.99, 30, "days", nil, &neg)
	require.Error(t, err)
	require.Contains(t, err.Error(), "period limit")
}

func TestValidatePlanRequired_ZeroPeriodLimit(t *testing.T) {
	zero := 0.0
	err := validatePlanRequired("Pro", 1, 9.99, 30, "days", nil, &zero)
	require.NoError(t, err)
}

func TestValidatePlanRequired_ValidPeriodLimit(t *testing.T) {
	limit := 500.0
	err := validatePlanRequired("Pro", 1, 9.99, 30, "days", nil, &limit)
	require.NoError(t, err)
}
