...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	usageHandler := handler.ProvideUsageHandler(usageService, apiKeyService, billingService, modelPricingResolver, userGroupRateRepository, settingService, opsService, db)
	redeemHandler := handler.NewRedeemHandler(redeemService)
	benefitRepository := repository.NewBenefitRepository(client)
	benefitService := service.ProvideBenefitService(benefitRepository, userRepository, billingCacheService, apiKeyAuthCacheInvalidator, client)
	benefitUserService := handler.ProvideBenefitUserService(benefitService)
	benefitHandler := handler.NewBenefitHandler(benefitUserService)
	...
	dashboardHandler := admin.NewDashboardHandler(dashboardService, dashboardAggregationService, concurrencyService, accountRepository)
	...
	adminBenefitHandler := admin.NewBenefitHandler(benefitService)
	...
	v := handler.ProvideSQLDBSlice(db)
	...
	paymentHandler := admin.ProvidePaymentHandler(paymentService, paymentConfigService, db)
	affiliateHandler := admin.NewAffiliateHandler(affiliateService, adminService)
	complianceHandler := admin.NewComplianceHandler(settingService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, adminBenefitHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, complianceHandler)
	...
	handlerPaymentHandler := handler.ProvidePaymentHandler(paymentService, paymentConfigService, channelService, db)
	paymentWebhookHandler := handler.NewPaymentWebhookHandler(paymentService, registry)
	availableChannelHandler := handler.NewAvailableChannelHandler(channelService, apiKeyService, settingService)
	publicModelPricingCatalogProvider := handler.ProvidePublicModelPricingCatalogProvider(pricingService)
	publicAvailableChannelProvider := handler.ProvidePublicAvailableChannelProvider(channelService)
	publicModelPricingHandler := handler.NewPublicModelPricingHandler(publicModelPricingCatalogProvider, publicAvailableChannelProvider)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, benefitHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, publicModelPricingHandler, idempotencyCoordinator, idempotencyCleanupService)
