...
func TestMigration147NormalizesRechargeRatioWithoutTouchingRealPaymentAmounts(t *testing.T) {
	content, err := FS.ReadFile("147_normalize_balance_recharge_ratio.sql")
	require.NoError(t, err)

	sql := string(content)
	require.Contains(t, sql, "v_ratio CONSTANT numeric := 7.5")
	require.Contains(t, sql, "balance_recharge_ratio_normalized_1_to_1")
	require.Contains(t, sql, "WHERE key = 'BALANCE_RECHARGE_MULTIPLIER'")
	require.Contains(t, sql, "WHERE key = 'BALANCE_RECHARGE_BONUS_TIERS'")
	require.Contains(t, sql, "bonus_amount")
	require.Contains(t, sql, "UPDATE users")
	require.Contains(t, sql, "balance = balance / v_ratio")
	require.Contains(t, sql, "total_recharged = total_recharged / v_ratio")
	require.Contains(t, sql, "UPDATE groups")
	require.Contains(t, sql, "rate_multiplier = rate_multiplier / v_ratio")
	require.Contains(t, sql, "image_rate_multiplier = image_rate_multiplier / v_ratio")
	require.Contains(t, sql, "UPDATE api_keys")
	require.Contains(t, sql, "quota = quota / v_ratio")
	require.Contains(t, sql, "quota_used = quota_used / v_ratio")
	require.Contains(t, sql, "quota_notify_daily_threshold")
	require.Contains(t, sql, "quota_notify_weekly_threshold")
	require.Contains(t, sql, "quota_notify_total_threshold")
	require.Contains(t, sql, "quota_notify_daily_threshold_type', 'fixed'")
	require.Contains(t, sql, "UPDATE user_platform_quotas")
	require.Contains(t, sql, "daily_limit_usd = daily_limit_usd / v_ratio")
	require.Contains(t, sql, "daily_usage_usd = daily_usage_usd / v_ratio")
	require.Contains(t, sql, "UPDATE payment_orders")
	require.Contains(t, sql, "SET amount = amount / v_ratio")
	require.Contains(t, sql, "refund_amount = refund_amount / v_ratio")
	require.Contains(t, sql, "WHERE order_type = 'balance'")
	require.Contains(t, sql, "UPDATE redeem_codes")
	require.Contains(t, sql, "SET value = value / v_ratio")
	...
	require.Contains(t, sql, "account_rate_multiplier = account_rate_multiplier / v_ratio")
	require.Contains(t, sql, "UPDATE usage_dashboard_hourly")
	require.Contains(t, sql, "UPDATE usage_dashboard_daily")
	require.Contains(t, sql, "account_cost = account_cost / v_ratio")

	require.NotContains(t, sql, "subscription_plans")
	require.NotContains(t, sql, "pay_amount =")
	require.NotContains(t, sql, "original_price")
	require.NotContains(t, sql, "total_cost = total_cost /")
	require.NotContains(t, sql, "input_cost = input_cost /")
	require.NotContains(t, sql, "output_cost = output_cost /")
	require.NotContains(t, sql, "discount_rate = discount_rate /")
}
