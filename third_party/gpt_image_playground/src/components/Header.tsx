          ...
          <div className="grid grid-cols-2 gap-1 rounded-xl border border-gray-200 dark:border-white/[0.08] bg-gray-100/70 dark:bg-white/[0.04] p-1 mx-2">
            {NAV_ITEMS.map((item) => (
              <button
                ...
                type="button"
                onClick={() => setAppMode(item.mode)}
                className={`px-2 py-1.5 rounded-lg text-sm transition-colors ${appMode === item.mode ? 'bg-white dark:bg-white/10 text-gray-900 dark:text-white shadow-sm font-medium' : 'text-gray-500 hover:text-gray-800 dark:hover:text-gray-200'}`}
              >
                {item.label}
              </button>
            ))}
