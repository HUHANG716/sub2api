...
import { IMAGE_FETCH_CORS_HINT, isImageGenerationDisabledForGroupError } from './lib/imageApiShared'
...
import { requestProductEmbedKeyRecreation } from './lib/productEmbedEvents'
...
export async function addTasksToFavoriteCollections(taskIds: string[], collectionIds: string[]) {
  const ids = normalizeFavoriteCollectionIds(collectionIds)
  const uniqueTaskIds = Array.from(new Set(taskIds)).filter(Boolean)
  if (!uniqueTaskIds.length || !ids.length) return
  const { tasks, setTasks, clearSelection, showToast } = useStore.getState()
  const idSet = new Set(uniqueTaskIds)
  const changedTaskIds = new Set<string>()
  const updated = tasks.map((task) => {
    if (!idSet.has(task.id)) return task
    const nextIds = normalizeFavoriteCollectionIds([...getTaskFavoriteCollectionIds(task), ...ids])
    if (sameFavoriteCollectionIds(getTaskFavoriteCollectionIds(task), nextIds)) return task
    changedTaskIds.add(task.id)
    return { ...task, favoriteCollectionIds: nextIds, isFavorite: true }
  })
  if (!changedTaskIds.size) {
    clearSelection()
    return
