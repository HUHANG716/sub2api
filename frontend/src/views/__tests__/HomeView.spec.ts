...
describe('HomeView', () => {
  ...
  it('keeps hero sticker collision boxes stable with depth-based parallax lanes', () => {
    ...
    expect(source).toContain('function heroTagStyle(tool: HeroFloatingTool): CSSProperties')
    ...
    expect(source).not.toContain("'--hero-tag-tilt-x'")
    expect(source).not.toContain("'--hero-tag-face-x'")
    expect(source).toContain("'--hero-tag-angle': `${tool.angle}deg`")
  ...
  it('styles the hero tool tags as oversized moving sticker logos with clean angled cards', () => {
    const source = readFileSync(homeViewSourcePath, 'utf8')
    ...
    expect(source).toContain("{ name: 'OpenAI', icon: '', needsBadge: false, shape: 'text', prominence: 'normal', angle: -3 }")
    ...
    expect(source).toContain("angle: -7")
    expect(source).toContain("angle: 6")
    expect(source).toContain('computeHeroTagLayout')
