...
import type { UsageLog, ModelStat, PlatformQuotaItem } from '@/types'
import { getMyPlatformQuotas } from '@/api/user'

const authStore = useAuthStore(); const user = computed(() => authStore.user)
const announcementStore = useAnnouncementStore()
const stats = ref<UserStatsType | null>(null); const loading = ref(false); const loadingUsage = ref(false); const loadingCharts = ref(false)
const modelStats = ref<ModelStat[]>([]); const recentUsage = ref<UsageLog[]>([])
...
const refreshAll = () => { loadStats(); loadCharts(); loadRecent(); loadPlatformQuotas(); loadAnnouncements() }
